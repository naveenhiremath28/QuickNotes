...
# 🧩 **1. Static Secrets Engines**

Like KV — store data exactly as you write it.
...
These engines simply store data and return it.

Nothing more.

---

# ⚙️ **2. Dynamic Secrets Engines**

These are the engines that make Vault _magic_.  
They can **generate secrets on the fly**, often with TTL (time-to-live), and revoke them automatically.

### Benefits:

- No long-lived passwords.
    
- Each application/user gets UNIQUE credentials.
    
- Auto-rotation is easy.
    
- If leaked, the secret is useless after TTL expires.
    

### Most popular dynamic engines:
...
- App asks Vault → Vault creates a DB user with TTL = 30m → App uses it → Vault deletes the DB user automatically → Zero static DB passwords.
...
### 1️⃣ One-time passwords (OTP)

Vault generates an _SSH password valid for 1 login_, then invalidates it.

### 2️⃣ Signed SSH certificates

Much better: Vault acts as a **CA**, signing SSH keys so no password is needed.

Real-world use:

- DevOps teams don’t share SSH keys.
    
- Vault signs a temporary key that expires.
...
Vault can generate short-lived credentials for:
...
- Vault creates temporary IAM credentials (STS creds)
...
Goodbye long-lived AWS keys in code.

---

# 🧩 **3. PKI & Crypto Engines**

These engines do not store secrets — they **create them using cryptography**.
...
- Internal HTTPS between microservices
    
- Rotating certificates automatically
    
- Kubernetes mTLS systems
...
This is **encryption-as-a-service**.

Vault **does NOT store your data** — it stores only the encryption keys.
...
- Encrypting PII before storing in DB
    
- Applications that must NOT manage crypto keys themselves
    

This is one of the most valuable engines in production systems.

---

# 🧩 **Other Important Secret Engines**

## 🔹 **JWT / OIDC Secret Engines**

Used to sign, verify, generate JWT tokens.

## 🔹 **Active Directory**

Vault can generate temporary Active Directory passwords for Windows servers.

---

# 🧠 **Why so many secret engines?**

Because every secret type has different needs.

|Secret Type|Static?|Dynamic?|Crypto?|
|---|---|---|---|
|KV Passwords|✔️|❌|❌|
|DB Credentials|❌|✔️|❌|
|SSH Keys|❌|✔️|❌|
|Certificates|❌|✔️|✔️|
|Application Data Encryption|❌|❌|✔️|

Vault is basically a **Swiss Army Knife** for security.

---

# 🧨 **The BIGGEST concept difference:**

### KV Engine

→ Store and fetch secrets (static)

### Other Engines (DB, SSH, AWS, PKI)

→ Generate secrets dynamically  
→ Revoke secrets automatically  
→ Rotate secrets automatically  
→ Enforce TTL  
→ Provide cryptographic services

This is what makes Vault much more than a “secret-store”.
...
✔ Dynamic engines  
✔ Crypto engines  
✔ Why dynamic secrets are extremely powerful  
✔ Why enterprise systems use Vault beyond just storing passwords
